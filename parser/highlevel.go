...
type Configuration struct {
	...
	Quirks     ConfigurationQuirks     `json:"quirks"`
...
type ConfigurationDecoration struct {
	...
	// how much the dim_around window rule should dim by. [0.0 - 1.0]
...
type ConfigurationDecorationBlur struct {
	...
	// works like ignore_alpha in layer rules. If pixel opacity is below set value, will not blur. [0.0 - 1.0]
...
type ConfigurationGroupGroupbar struct {
	...
	// adjusts the curve used for rounding groupbar corners, larger is smoother, 2.0 is a circle, 4.0 is a squircle, 1.0 is a triangular corner. [1.0 - 10.0]
	RoundingPower float32 `json:"rounding_power"`
...
type ConfigurationMisc struct {
	...
	OnFocusUnderFullscreen int `json:"on_focus_under_fullscreen"`
...
type ConfigurationCursor struct {
	...
	ZoomDetachedCamera bool `json:"zoom_detached_camera"`
...
type ConfigurationQuirks struct {
	// Report HDR mode as preferred. 0 - off, 1 - always, 2 - gamescope only
	PreferHdr int `json:"prefer_hdr"`
}
